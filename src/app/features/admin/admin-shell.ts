...
@Component({
  ...
  template: `
    ...
    <aside class="w-64 h-screen fixed left-0 top-0 z-40 flex flex-col justify-between p-4 bg-surface-container-lowest border-r border-outline-variant/30 shadow-sm transition-transform duration-200 lg:translate-x-0" [class.-translate-x-full]="!open()">
      <div class="flex flex-col gap-6">
        <div class="flex items-center gap-3 px-2 py-1">
          ...
          <div class="flex flex-col"><span class="font-headline-md text-headline-md font-bold text-primary tracking-tight leading-tight">Chairly</span><span class="font-label-sm text-label-sm text-outline font-medium tracking-wide">Super Admin</span></div>
        </div>
        <nav aria-label="Admin navigation" class="flex flex-col gap-1.5">
          @for (n of nav; track n.path) {
            <a [routerLink]="n.path" routerLinkActive #rla="routerLinkActive" (click)="open.set(false)" class="flex items-center gap-3 px-3 py-2.5 rounded-lg font-label-lg text-label-lg transition-colors" [class]="rla.isActive ? 'bg-primary/10 text-primary border-l-4 border-primary' : 'text-on-surface-variant hover:bg-surface-container hover:text-on-surface'">
              <span class="material-symbols-outlined" [style.font-variation-settings]="rla.isActive ? '\\'FILL\\' 1' : null">{{ n.icon }}</span><span>{{ n.label }}</span>
            </a>
          }
        </nav>
      </div>
      <div class="pt-4 border-t border-outline-variant/30 flex items-center gap-3 px-3 py-2">
        ...
        <div class="flex flex-col overflow-hidden"><span class="font-label-lg text-label-lg truncate text-on-surface">Chairly Admin</span><span class="font-label-sm text-label-sm text-outline truncate">{{ auth.user().email }}</span></div>
      </div>
    </aside>
    ...
    <header class="h-16 fixed top-0 right-0 left-0 lg:left-64 z-30 flex items-center gap-3 px-4 md:px-6 bg-surface-container-lowest border-b border-outline-variant/30">
      <button type="button" class="lg:hidden p-2 -ml-2 rounded-lg text-on-surface-variant hover:bg-surface-container" aria-label="Open menu" (click)="open.set(true)"><span class="material-symbols-outlined">menu</span></button>
      <span class="font-headline-sm text-headline-sm text-on-surface">Platform Control Center</span>
      <span class="ml-auto hidden sm:inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-primary/10 text-primary font-label-sm text-label-sm"><span class="w-2 h-2 rounded-full bg-primary animate-pulse-dot"></span> All systems normal</span>
    </header>
