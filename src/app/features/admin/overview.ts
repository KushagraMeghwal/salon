    ...
    <div class="p-4 md:p-8 flex flex-col gap-6 max-w-[1400px] mx-auto">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        <div class="lg:col-span-8 bg-surface-container-lowest p-5 md:p-6 rounded-2xl border border-outline-variant/20 shadow-level-1">
          <div class="flex items-center justify-between mb-4"><div><h2 class="font-headline-sm text-headline-sm font-bold text-on-surface">New Salon Signups</h2><p class="font-body-sm text-body-sm text-tertiary">Last 8 weeks</p></div><span class="px-2.5 py-1 rounded-full bg-primary/10 text-primary font-label-sm text-label-sm font-bold">{{ signupTotal }} total</span></div>
          <div class="h-56 w-full pt-2"><app-line-chart [series]="[{ values: store.signups, color: '#00685b', area: true, marker: true }]" /></div>
          <div class="flex justify-between text-tertiary font-label-sm text-label-sm pt-3 border-t border-outline-variant/20 mt-2">@for (w of weeks; track w) { <span>{{ w }}</span> }</div>
        </div>
        ...
        <div class="lg:col-span-4 bg-surface-container-lowest p-6 rounded-2xl border border-outline-variant/20 shadow-level-1">
          ...
          <p class="font-body-sm text-body-sm text-tertiary mb-2">Active subscribers by plan</p>
          <app-donut-chart [segments]="donut()"><span class="font-label-sm text-label-sm text-tertiary">Subscribers</span><span class="font-headline-md text-headline-md font-bold text-on-surface">{{ store.totals().subscribers }}</span></app-donut-chart>
          ...
          <div class="flex flex-col gap-2.5">
            @for (t of expiring(); track t.id) {
              <div class="p-3 rounded-xl bg-surface-container-low/40 border border-outline-variant/20 flex items-center justify-between gap-2">
                <div class="min-w-0"><p class="font-label-lg text-label-lg font-bold text-on-surface truncate">{{ t.name }}</p><p class="text-body-sm text-tertiary">{{ t.owner }} • {{ t.city }}</p></div>
        ...
        <div class="bg-surface-container-lowest p-5 md:p-6 rounded-2xl border border-outline-variant/20 shadow-level-1">
          ...
          <div class="flex flex-col gap-2.5">
            @for (r of recent(); track r.id) {
              <div class="p-3 rounded-xl bg-surface-container-low/40 border border-outline-variant/20 flex items-center justify-between gap-2">
                <div class="min-w-0"><p class="font-label-lg text-label-lg font-bold text-on-surface truncate">{{ r.name }}</p><p class="text-body-sm text-tertiary">{{ r.city }} • {{ r.staff }} staff</p></div>
...
export class AdminOverview {
  ...
  protected readonly kpis = computed(() => {
    ...
    return [
      ...
      { label: 'Subscribers', value: t.subscribers, icon: 'verified', tone: 'bg-surface-container text-tertiary', note: `${Math.round((t.subscribers / Math.max(1, t.salons)) * 100)}% of all salons` },
