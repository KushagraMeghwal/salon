...
@Component({
  ...
  imports: [RouterLink, LineChart, DonutChart, TranslatePipe],
  template: `
    <div class="p-4 md:p-8 flex flex-col gap-6 max-w-[1400px] mx-auto">
      <div>
        <h1 class="font-headline-lg text-headline-lg-mobile md:text-headline-lg text-on-surface tracking-tight">{{ "Overview" | translate }}</h1>
        <p class="font-body-md text-body-md text-outline">{{ "Salons, trials, subscribers and recurring revenue across Chairly." | translate }}</p>
      </div>
        ...
        @for (k of kpis(); track k.label) {
          <div class="p-5 rounded-[16px] bg-surface-container-lowest border border-outline-variant/30 shadow-level-1 hover:shadow-level-2 transition-all flex flex-col justify-between">
            <div class="flex items-start justify-between">
              <div class="flex flex-col"><span class="font-label-md text-label-md text-outline font-medium">{{ (k.label) | translate }}</span><span class="font-headline-xl text-headline-xl-mobile md:text-headline-xl text-on-surface tracking-tight mt-1">{{ k.value }}</span></div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div class="lg:col-span-4 bg-surface-container-lowest p-6 rounded-2xl border border-outline-variant/20 shadow-level-1">
          <h2 class="font-headline-sm text-headline-sm font-bold text-on-surface">{{ "Plan Mix" | translate }}</h2>
          <p class="font-body-sm text-body-sm text-muted mb-2">{{ "Active subscribers by plan" | translate }}</p>
          <app-donut-chart [segments]="donut()"><span class="font-label-sm text-label-sm text-muted">{{ "Subscribers" | translate }}</span><span class="font-headline-md text-headline-md font-bold text-on-surface">{{ store.totals().subscribers }}</span></app-donut-chart>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="bg-surface-container-lowest p-5 md:p-6 rounded-2xl border border-outline-variant/20 shadow-level-1">
          <div class="flex items-center justify-between mb-4"><h2 class="font-headline-sm text-headline-sm font-bold text-on-surface">{{ "Trials ending soon" | translate }}</h2><a routerLink="/admin/salons" class="text-primary font-label-md text-label-md font-semibold hover:underline">{{ "View all" | translate }}</a></div>
          <div class="flex flex-col gap-2.5">
            @for (t of expiring(); track t.id) {
              <div class="p-3 rounded-xl bg-surface-container-low/40 border border-outline-variant/20 flex items-center justify-between gap-2">
                ...
                <span class="px-2.5 py-1 rounded-full font-label-sm text-label-sm font-semibold shrink-0" [class]="store.daysLeft(t.trialEndsAt) <= 3 ? 'bg-error-container text-on-error-container' : 'bg-amber-100 text-amber-900'">{{ (store.daysLeft(t.trialEndsAt) === 1 ? "{{p1}} day left" : "{{p1}} days left") | translate: { p1: store.daysLeft(t.trialEndsAt) } }}</span>
              </div>
            } @empty { <p class="text-body-sm text-outline py-4">{{ "No trials ending in the next 7 days." | translate }}</p> }
          </div>
        </div>
        <div class="bg-surface-container-lowest p-5 md:p-6 rounded-2xl border border-outline-variant/20 shadow-level-1">
          <div class="flex items-center justify-between mb-4"><h2 class="font-headline-sm text-headline-sm font-bold text-on-surface">{{ "Recently joined" | translate }}</h2></div>
          <div class="flex flex-col gap-2.5">
            @for (r of recent(); track r.id) {
              <div class="p-3 rounded-xl bg-surface-container-low/40 border border-outline-variant/20 flex items-center justify-between gap-2">
                <div class="min-w-0"><p class="font-label-lg text-label-lg font-bold text-on-surface truncate">{{ r.name }}</p><p class="text-body-sm text-muted">{{ "{{p1}} • {{p2}} staff" | translate: { p1: (r.city), p2: (r.staff) } }}</p></div>
...
export class AdminOverview {
  ...
  protected readonly kpis = computed(() => {
    ...
    return [
      { label: 'Total Salons', value: t.salons, icon: 'storefront', tone: 'bg-primary/10 text-primary', note: tr('{{p1}} suspended or expired', { p1: t.suspended }) },
      { label: 'On Free Trial', value: t.trials, icon: 'hourglass_top', tone: 'bg-amber-500/10 text-amber-700', note: tr('14-day trial on signup') },
      { label: 'Subscribers', value: t.subscribers, icon: 'verified', tone: 'bg-surface-container text-muted', note: tr('{{p1}}% of all salons', { p1: Math.round((t.subscribers / Math.max(1, t.salons)) * 100) }) },
      { label: 'Monthly Revenue', value: inr(t.mrr), icon: 'payments', tone: 'bg-secondary/10 text-secondary', note: tr('Recurring, from active plans') },
    ];
  });
