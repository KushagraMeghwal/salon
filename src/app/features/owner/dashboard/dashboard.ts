    ...
    <app-topbar>
      <div left class="flex items-center gap-4 flex-1 max-w-lg">
        <div class="relative w-full max-w-xs">
          ...
          <input type="text" class="w-full pl-10 pr-3 py-1.5 bg-surface-container-low border border-outline-variant/50 rounded-xl text-on-surface font-body-md text-body-md placeholder:text-outline focus:bg-surface-container-lowest focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all" placeholder="Search client or service..." aria-label="Search queue" [ngModel]="search()" (ngModelChange)="search.set($event)" />
        </div>
      </div>
      <ng-container right>
        <button type="button" class="w-9 h-9 rounded-xl flex items-center justify-center text-on-surface-variant hover:bg-surface-container transition-colors relative" title="Notifications" (click)="notify()">
        ...
        <button type="button" class="hidden sm:flex w-9 h-9 rounded-xl items-center justify-center text-on-surface-variant hover:bg-surface-container transition-colors" title="Support & Help" (click)="toast.info('Partner support: support@chairly.app')">
        ...
        <button type="button" (click)="ui.walkInModal.set(true)" class="px-3.5 py-2 rounded-xl bg-surface-container hover:bg-surface-variant/40 text-primary font-label-lg text-label-lg border border-primary/20 flex items-center gap-1.5 transition-all active:scale-95">
          <span class="material-symbols-outlined text-[18px]">person_add</span><span class="hidden sm:inline">Add Walk-in</span>
        </button>
        <a routerLink="/owner/quick-bill" class="px-4 py-2 rounded-xl bg-secondary-container hover:bg-secondary text-on-secondary-container hover:text-on-secondary font-label-lg text-label-lg flex items-center gap-2 shadow-sm transition-all duration-150 active:scale-95">
          <span class="material-symbols-outlined text-[18px]">receipt_long</span><span class="hidden sm:inline">Quick Bill</span>
        </a>
      </ng-container>
    </app-topbar>
    ...
    <main class="lg:pl-64 pt-16 min-h-screen flex flex-col bg-background">
      <div class="p-4 md:p-8 flex flex-col gap-6 max-w-[1600px] w-full mx-auto">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h1 class="font-headline-lg text-headline-lg-mobile md:text-headline-lg text-on-surface tracking-tight">Salon Owner Command Center</h1>
            <p class="font-body-md text-body-md text-outline">Real-time floor oversight, queue tracking, and revenue for today, {{ today }}.</p>
          </div>
          <div class="flex items-center gap-2 self-start sm:self-auto">
            <div class="flex items-center gap-2 bg-surface-container-lowest px-3 py-1.5 rounded-xl border border-outline-variant/30 shadow-level-1">
              ...
              <span class="font-label-md text-label-md text-on-surface font-medium">Floor Sync: Live</span>
              <span class="text-outline text-xs">• {{ fmt(store.nowMin()) }}</span>
            </div>
            <button type="button" class="p-2 rounded-xl bg-surface-container-lowest border border-outline-variant/30 text-on-surface-variant hover:text-primary transition-colors shadow-level-1" title="Refresh Live Data" (click)="toast.success('Live data refreshed')">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5">
          <div class="p-5 rounded-[16px] bg-surface-container-lowest border border-outline-variant/30 shadow-level-1 hover:shadow-level-2 transition-all duration-200 flex flex-col justify-between">
            <div class="flex items-start justify-between">
              <div class="flex flex-col">
                <span class="font-label-md text-label-md text-outline font-medium">Today's Earnings</span>
            ...
            <div class="mt-4 pt-3 border-t border-outline-variant/20 flex items-center justify-between">
              <div class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-primary/10 text-primary font-label-sm text-label-sm font-semibold whitespace-nowrap"><span class="material-symbols-outlined text-[14px]">trending_up</span><span>+14% vs yesterday</span></div>
          ...
          <div class="p-5 rounded-[16px] bg-surface-container-lowest border border-outline-variant/30 shadow-level-1 hover:shadow-level-2 transition-all duration-200 flex flex-col justify-between">
            ...
            <div class="mt-4 pt-3 border-t border-outline-variant/20 flex flex-wrap items-center justify-between gap-x-3 gap-y-1 font-label-sm text-label-sm">
              ...
              <div class="flex items-center gap-1.5 text-secondary"><span class="w-2 h-2 rounded-full bg-secondary-container"></span><span class="font-semibold">{{ totalBookings() - doneCount() }}</span> Pending</div>
              <div class="text-outline">{{ concluded() }}% Concluded</div>
            </div>
          </div>
          ...
          <div class="p-5 rounded-[16px] bg-surface-container-lowest border border-outline-variant/30 shadow-level-1 hover:shadow-level-2 transition-all duration-200 flex flex-col justify-between">
            ...
            <div class="mt-4 pt-3 border-t border-outline-variant/20 flex flex-col gap-1.5">
              ...
              <div class="flex items-center justify-between font-label-sm text-label-sm text-outline">
                <span>Walk-ins: <b class="text-primary">{{ walkinPct() }}%</b></span>
                <span>Online: <b class="text-secondary">{{ 100 - walkinPct() }}%</b></span>
              </div>
            </div>
          </div>
          ...
          <div class="p-5 rounded-[16px] bg-surface-container-lowest border border-outline-variant/30 shadow-level-1 hover:shadow-level-2 transition-all duration-200 flex flex-col justify-between">
            <div class="flex items-start justify-between">
              <div class="flex flex-col">
                <span class="font-label-md text-label-md text-outline font-medium">Payment Split</span>
            ...
            <div class="mt-4 pt-3 border-t border-outline-variant/20 flex flex-wrap items-center justify-between gap-x-3 gap-y-1 font-label-sm text-label-sm">
              <div class="flex items-center gap-1.5 text-primary font-semibold"><span class="material-symbols-outlined text-[15px]">qr_code_2</span><span>UPI: {{ inr(upi()) }}</span></div>
              <div class="flex items-center gap-1.5 text-outline font-medium"><span class="material-symbols-outlined text-[15px]">local_atm</span><span>Cash: {{ 100 - upiPct() }}%</span></div>
            </div>
          </div>
        </div>
          ...
          <div class="xl:col-span-8 bg-surface-container-lowest rounded-[16px] border border-outline-variant/30 shadow-level-1 p-4 md:p-6 flex flex-col gap-6">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-outline-variant/20 pb-4">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <h2 class="font-headline-md text-headline-md text-on-surface tracking-tight">Today's Salon Queue</h2>
                  <p class="font-body-sm text-body-sm text-outline">Real-time station allocations and service stage transitions</p>
                </div>
              </div>
              <div class="flex items-center gap-1 bg-surface-container-low p-1 rounded-xl border border-outline-variant/30 overflow-x-auto no-scrollbar">
                <button type="button" (click)="filter.set('All')" class="px-3 py-1 rounded-lg font-label-sm text-label-sm whitespace-nowrap transition-colors" [class]="filter() === 'All' ? 'bg-surface-container-lowest text-primary font-semibold shadow-xs' : 'text-outline hover:text-on-surface font-medium'">All Stations ({{ store.staff().length }})</button>
                @for (c of categories(); track c) {
                  <button type="button" (click)="filter.set(c)" class="px-3 py-1 rounded-lg font-label-sm text-label-sm whitespace-nowrap transition-colors" [class]="filter() === c ? 'bg-surface-container-lowest text-primary font-semibold shadow-xs' : 'text-outline hover:text-on-surface font-medium'">{{ c }}</button>
                }
              </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div class="flex flex-col gap-3 bg-surface-container-low/60 rounded-xl p-3 border border-outline-variant/20">
                <div class="flex items-center justify-between px-1">
                  <div class="flex items-center gap-2"><span class="w-2.5 h-2.5 rounded-full bg-amber-500"></span><span class="font-label-lg text-label-lg font-bold text-on-surface">Waiting</span></div>
                  <span class="px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-800 text-label-sm font-bold">{{ waiting().length }} clients</span>
                </div>
                @for (q of waiting(); track q.id) {
                  <div class="p-3.5 rounded-xl bg-surface-container-lowest border border-outline-variant/30 shadow-xs hover:shadow-level-2 transition-all duration-150 flex flex-col gap-2">
                    ...
                    <div class="pt-2 border-t border-outline-variant/20 flex items-center justify-between">
                      <span class="font-label-sm text-label-sm text-outline">{{ q.requestedStaffId ? 'Requested: ' + first(q.requestedStaffId) : 'Any Stylist' }}</span>
                      <button type="button" (click)="seat(q)" class="text-primary hover:text-primary-container font-label-sm text-label-sm font-semibold flex items-center gap-0.5">
                        {{ q.requestedStaffId ? 'Assign' : 'Seat Client' }} <span class="material-symbols-outlined text-[14px]">{{ q.requestedStaffId ? 'arrow_forward' : 'chair' }}</span>
                      </button>
                    </div>
                  </div>
                } @empty {
                  <p class="text-center text-body-sm text-outline py-6">No one waiting</p>
                }
              </div>

              <div class="flex flex-col gap-3 bg-surface-container-low/60 rounded-xl p-3 border border-primary/20 ring-1 ring-primary/10">
                <div class="flex items-center justify-between px-1">
                  <div class="flex items-center gap-2"><span class="w-2.5 h-2.5 rounded-full bg-primary animate-pulse-dot"></span><span class="font-label-lg text-label-lg font-bold text-on-surface">In Chair</span></div>
                  <span class="px-2 py-0.5 rounded-full bg-primary/10 text-primary text-label-sm font-bold">{{ inChair().length }} active</span>
                </div>
                  ...
                  <div class="p-3.5 rounded-xl bg-surface-container-lowest border-l-4 border-l-primary border-t border-r border-b border-outline-variant/30 shadow-xs hover:shadow-level-2 transition-all flex flex-col gap-2">
                    <div class="flex items-start justify-between gap-2">
                      <div class="min-w-0">
                        ...
                        <span class="font-label-sm text-label-sm text-primary font-semibold flex items-center gap-1 mt-0.5"><span class="material-symbols-outlined text-[14px]">event_seat</span> {{ first(q.staffId) }} - Station {{ q.station }}</span>
                      </div>
                    ...
                    <div class="pt-1.5 flex items-center justify-between">
                      ...
                      <a [routerLink]="'/owner/quick-bill'" [queryParams]="{ queueId: q.id }" class="text-secondary hover:text-on-secondary-container font-label-sm text-label-sm font-semibold">{{ progress(q) >= 80 ? 'Ready to checkout' : 'Generate Bill' }}</a>
                    </div>
                  </div>
                } @empty {
                  <p class="text-center text-body-sm text-outline py-6">All chairs are free</p>
                }
              </div>

              <div class="flex flex-col gap-3 bg-surface-container-low/60 rounded-xl p-3 border border-outline-variant/20">
                <div class="flex items-center justify-between px-1">
                  <div class="flex items-center gap-2"><span class="w-2.5 h-2.5 rounded-full bg-emerald-600"></span><span class="font-label-lg text-label-lg font-bold text-on-surface">Done</span></div>
                  <span class="px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 text-label-sm font-bold">{{ doneCount() }} billed</span>
                </div>
                @for (q of done(); track q.id) {
                  ...
                  <p class="text-center text-body-sm text-outline py-6">No bills yet</p>
                }
              </div>
            </div>
          </div>
          ...
          <div class="xl:col-span-4 flex flex-col gap-6">
            <div class="bg-surface-container-lowest rounded-[16px] border border-outline-variant/30 shadow-level-1 p-5 flex flex-col gap-4">
              <div class="flex items-center justify-between gap-2">
                <div><h3 class="font-headline-sm text-headline-sm text-on-surface">Footfall &amp; Rush Hours</h3><p class="font-body-sm text-body-sm text-outline">Revenue spikes across operational shifts</p></div>
                <span class="px-2.5 py-1 rounded-full bg-primary/10 text-primary font-label-sm text-label-sm font-bold whitespace-nowrap">Peak: {{ peak() }}</span>
              </div>
                ...
                <div class="h-36 w-full flex items-end justify-between gap-2 px-1">
                  @for (b of rush(); track b.label) {
                    <div class="flex-1 flex flex-col items-center gap-1.5 group cursor-pointer">
                      ...
                      <span class="text-[11px] font-medium" [class]="b.now ? 'text-primary font-bold' : 'text-outline'">{{ b.now ? 'Now' : b.label }}</span>
                    </div>
                  }
                </div>
                <div class="flex items-center justify-between text-label-sm text-outline px-1 border-t border-outline-variant/20 pt-2">
                  <div class="flex items-center gap-1.5"><span class="w-2 h-2 rounded-full bg-primary"></span><span>Current shift</span></div>
                  <div class="flex items-center gap-1.5"><span class="w-2 h-2 rounded-full bg-primary-container/80"></span><span>Peak shift</span></div>
                </div>
              </div>
            </div>
            ...
            @if (topStylist(); as t) {
              <div class="bg-surface-container-lowest rounded-[16px] border border-outline-variant/30 shadow-level-1 p-4 flex items-center justify-between gap-3 hover:shadow-level-2 transition-all">
                ...
                <div class="flex flex-col items-end shrink-0"><span class="font-headline-sm text-headline-sm text-primary font-bold">{{ inr(t.stats.revenue) }}</span><span class="font-label-sm text-label-sm text-emerald-700 bg-emerald-50 px-1.5 py-0.5 rounded font-medium">This month</span></div>
              </div>
            }
          </div>
        </div>
...
export class Dashboard {
  ...
  protected readonly today = new Date().toLocaleDateString('en-IN', { day: 'numeric', month: 'short' });
  ...
  billTime(q: QueueItem) {
    return q.billedAt ? new Date(q.billedAt).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' }) : '';
  }
  ...
  seat(q: QueueItem) {
    ...
    this.toast.success(`${q.client} seated with ${this.first(updated?.staffId)} (Station ${updated?.station})`);
  }

  notify() {
    const n = this.waiting().length;
    this.toast.info(n ? `${n} client${n > 1 ? 's' : ''} waiting for a chair` : 'No new notifications');
  }
