...
@Component({
  ...
  imports: [RouterLink, WizardHeader, SalonMark],
  template: `
          ...
          <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative z-10">
            ...
            <div class="flex items-center gap-3 w-full md:w-auto justify-end border-t md:border-t-0 pt-4 md:pt-0 border-outline-variant/20">
              <div class="flex gap-2">
                <span class="px-3 py-1 bg-surface-container rounded-lg text-label-md font-label-md text-on-surface-variant font-medium">{{ store.staff().length }} Stylists</span>
                <span class="px-3 py-1 bg-surface-container rounded-lg text-label-md font-label-md text-on-surface-variant font-medium">{{ store.selectedServices().length }} Services</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="w-full max-w-5xl grid grid-cols-1 md:grid-cols-12 gap-6 mb-8">
          <div class="md:col-span-5 bg-surface-container-lowest rounded-2xl p-6 border border-outline-variant/30 shadow-level-1 flex flex-col justify-between hover:shadow-level-2 transition-all duration-200">
            <div>
              <div class="flex items-center justify-between mb-4">
                <span class="inline-flex items-center gap-1.5 text-label-sm font-label-sm font-semibold tracking-wider text-primary uppercase"><span class="material-symbols-outlined text-[16px]">qr_code_scanner</span> Reception Asset</span>
              ...
              <div class="p-5 bg-surface-container-low rounded-xl border border-outline-variant/30 flex flex-col items-center justify-center">
                <div class="relative bg-surface-container-lowest p-3.5 rounded-lg shadow-sm border border-outline-variant/20 flex items-center justify-center">
                  @if (qr()) {
                    <img [src]="qr()" alt="Booking QR code" class="w-44 h-44" />
          ...
          <div class="md:col-span-7 flex flex-col gap-6">
            <div class="bg-surface-container-lowest rounded-2xl p-6 border border-outline-variant/30 shadow-level-1 hover:shadow-level-2 transition-all duration-200">
              ...
              <div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                  ...
                  <a class="flex items-center justify-center gap-2 p-2.5 rounded-xl border border-outline-variant/30 hover:border-primary hover:bg-primary/5 transition-all text-on-surface group" [href]="sms()"><span class="material-symbols-outlined text-[18px] text-primary group-hover:scale-110 transition-transform">sms</span><span class="text-label-md font-label-md font-medium">SMS Blast</span></a>
                  <a class="flex items-center justify-center gap-2 p-2.5 rounded-xl border border-outline-variant/30 hover:border-primary hover:bg-primary/5 transition-all text-on-surface group" [href]="facebook()" target="_blank" rel="noopener"><span class="material-symbols-outlined text-[18px] text-blue-600 group-hover:scale-110 transition-transform">share</span><span class="text-label-md font-label-md font-medium">Facebook</span></a>
                </div>
              </div>
            </div>
            ...
            <div class="bg-surface-container-lowest rounded-2xl p-6 border border-outline-variant/30 shadow-level-1">
              <div class="flex items-center justify-between mb-4">
                <h3 class="text-headline-sm font-headline-sm text-on-surface flex items-center gap-2"><span class="material-symbols-outlined text-primary text-[20px]">checklist</span> What happens next?</h3>
                <span class="text-label-sm font-label-sm text-primary font-semibold bg-primary/10 px-2 py-0.5 rounded">3 Quick Steps</span>
              </div>
...
export class SetupDone implements OnInit {
  ...
  async copyFor(channel: string) {
    await this.writeClipboard();
    this.toast.success(`Link copied. Paste it in your ${channel} bio.`);
  }
