...
@Component({
  ...
  imports: [FormsModule, WizardHeader, Modal],
  template: `
                  ...
                  <div class="flex items-start justify-between gap-3">
                    <div class="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span class="inline-block mt-0.5 text-label-sm font-label-sm px-2 py-0.5 rounded bg-surface-container text-primary">{{ s.category }}</span>
                      </div>
                    </div>
      ...
      <footer class="sticky bottom-0 z-40 bg-surface-container-lowest border-t border-outline-variant/20 shadow-md">
        <div class="max-w-7xl mx-auto px-4 py-4 md:px-12 flex items-center justify-between gap-3">
          <button type="button" (click)="back()" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg border border-outline-variant/40 text-on-surface text-label-lg font-label-lg hover:bg-surface-container-low transition-colors duration-150 active:scale-[0.99]">
            ...
            <span class="hidden sm:inline">Back to Salon Details</span><span class="sm:hidden">Back</span>
          </button>
          <button type="button" (click)="next()" class="inline-flex items-center gap-2 px-6 py-2.5 rounded-lg bg-primary text-on-primary text-label-lg font-label-lg hover:bg-primary-container shadow-sm hover:shadow transition-all duration-150 active:scale-[0.99]">
            <span class="hidden sm:inline">Continue to Timings &amp; Slots</span><span class="sm:hidden">Continue</span>
            <span class="material-symbols-outlined text-[18px]">arrow_forward</span>
          </button>
        </div>
      </footer>

      <app-modal [open]="modal()" title="Add Custom Service" (closed)="modal.set(false)">
        <form class="space-y-4" (ngSubmit)="saveCustom()" #f="ngForm">
          <div>
            <label [class]="label" for="cs-name">Custom Service Name</label>
            <input id="cs-name" name="name" [class]="input" placeholder="e.g., Ayurvedic Hair Spa" [(ngModel)]="draft.name" required />
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label [class]="label" for="cs-cat">Category</label>
          ...
          <div>
            <label [class]="label" for="cs-dur">Duration in mins</label>
            <input id="cs-dur" name="duration" type="number" min="5" step="5" [class]="input" placeholder="45" [(ngModel)]="draft.duration" required />
          </div>
          <div>
            <label [class]="label" for="cs-desc">Description</label>
            <textarea id="cs-desc" name="description" rows="3" [class]="input" placeholder="Brief outline of steps, products used, or prerequisites..." [(ngModel)]="draft.description"></textarea>
          </div>
          <div class="flex items-center justify-end gap-3 pt-4 border-t border-outline-variant/20">
            <button type="button" [class]="ghost" (click)="modal.set(false)">Cancel</button>
            <button type="submit" [class]="primary" [disabled]="f.invalid">Save Service</button>
          </div>
        </form>
      </app-modal>
    </div>
...
export class ServicesStep {
  ...
  saveCustom() {
    ...
    this.toast.success(`"${d.name.trim()}" added`);
