    ...
    <app-topbar>
      ...
      <ng-container right>
        ...
        <div class="flex items-center gap-2.5 pl-1">
          ...
          <div class="hidden xl:flex flex-col text-left"><span class="font-label-md text-label-md leading-tight text-on-surface font-semibold">{{ auth.user().name }}</span><span class="font-label-sm text-label-sm text-muted">{{ auth.user().title }}</span></div>
        </div>
      </ng-container>
    </app-topbar>
        ...
        <div class="flex-1 flex flex-col gap-5 min-w-0">
          <div class="bg-surface-container-lowest rounded-2xl p-5 border border-outline-variant/30 shadow-level-1 flex flex-col gap-4">
            <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
              ...
              <div class="md:col-span-4 flex flex-col gap-1.5">
                <label class="font-label-sm text-label-sm text-muted" for="qb-phone">Mobile Number</label>
                <div class="relative">
              ...
              <div class="md:col-span-3 flex md:flex-col justify-between items-start md:items-end gap-1 pt-1">
                @if (knownCustomer(); as k) {
                  <span class="font-label-sm text-label-sm text-muted">Returning client</span>
                  <span class="text-body-sm text-primary font-medium">{{ k.visits }} visits · {{ inr(k.totalSpent) }} spent</span>
                }
              </div>
            </div>
              ...
              <div class="flex flex-wrap items-center gap-2.5">
                @for (s of store.staff(); track s.id) {
                  <button type="button" (click)="staffId.set(s.id)" class="flex items-center gap-2 px-3 py-1.5 rounded-full font-label-md text-label-md transition-all border" [class]="staffId() === s.id ? 'bg-primary text-on-primary shadow-xs border-primary' : 'bg-surface-container hover:bg-surface-variant/50 text-on-surface border-outline-variant/30'">
                    ...
                    <span>{{ s.name }}</span><span class="text-[10px] font-normal" [class]="staffId() === s.id ? 'opacity-80' : 'text-muted'">{{ s.role }}</span>
                  </button>
                }
              </div>
            </div>
          ...
          <div class="flex flex-col gap-3">
            <div class="flex items-center justify-between overflow-x-auto no-scrollbar pb-1 gap-2">
              ...
              <div class="hidden sm:flex items-center gap-1.5 text-muted font-label-sm text-label-sm pl-2 whitespace-nowrap"><span class="material-symbols-outlined text-[16px]">touch_app</span> Tap to add to invoice</div>
            </div>
                ...
                <div class="rounded-2xl p-4 relative flex flex-col justify-between min-h-44 transition-all group bg-surface-container-lowest" [class]="inCart(s) ? 'border-2 border-primary shadow-level-2' : 'border border-outline-variant/30 hover:border-primary/60 shadow-level-1 hover:shadow-level-2'">
                  ...
                  <div>
                    <div class="flex items-center gap-2 mb-2">
                      ...
                      <span class="font-label-sm text-label-sm font-semibold" [class]="inCart(s) ? 'text-primary' : 'text-muted'">{{ s.category }}</span>
                    </div>
                    ...
                    <div class="flex items-center gap-1.5 text-muted font-body-sm text-body-sm mt-1"><span class="material-symbols-outlined text-[15px]">schedule</span><span>{{ s.duration }} mins</span></div>
                  </div>
        ...
        <div class="w-full xl:w-[420px] flex flex-col gap-4 shrink-0">
          <div class="bg-surface-container-lowest rounded-2xl p-5 border border-outline-variant/30 shadow-level-2 flex flex-col gap-5 xl:sticky xl:top-20">
                ...
                <div class="flex items-start justify-between gap-2 p-3 rounded-xl bg-surface-bright border border-outline-variant/30">
                  ...
                  <div class="flex flex-col items-end gap-1.5 shrink-0">
                    ...
                    <div class="flex items-center border border-outline-variant/40 rounded-lg bg-surface-container-lowest">
                      <button type="button" aria-label="Decrease quantity" class="w-6 h-6 flex items-center justify-center text-muted hover:text-on-surface text-xs" (click)="qty(l, -1)">-</button>
                      <span class="w-6 text-center text-xs font-semibold">{{ l.qty }}</span>
                      <button type="button" aria-label="Increase quantity" class="w-6 h-6 flex items-center justify-center text-muted hover:text-on-surface text-xs" (click)="qty(l, 1)">+</button>
                    </div>
                  </div>
                </div>
              } @empty {
            ...
            <div class="flex gap-2">
              ...
              <button type="button" (click)="applied() ? removeCoupon() : applyCoupon()" class="px-3.5 py-2 bg-surface-container hover:bg-surface-variant text-on-surface font-label-md text-label-md rounded-xl font-semibold border border-outline-variant/40 transition-colors">{{ applied() ? 'Remove' : 'Apply' }}</button>
            </div>

            <div class="flex flex-col gap-2 pt-3 border-t border-outline-variant/20 font-body-sm text-body-sm">
              <div class="flex justify-between text-muted"><span>Subtotal ({{ count() }} service{{ count() === 1 ? '' : 's' }})</span><span class="font-medium text-on-surface">{{ inr(subtotal()) }}</span></div>
              @if (discount() > 0) {
                <div class="flex justify-between text-primary font-medium"><span class="flex items-center gap-1"><span class="material-symbols-outlined text-[14px]">sell</span> Coupon {{ applied()!.code }}</span><span>-{{ inr(discount()) }}</span></div>
              }
              @if (split().gst > 0) {
                <div class="flex justify-between text-muted text-[12px]"><span>Taxable value</span><span>{{ inr(split().taxable) }}</span></div>
                <div class="flex justify-between text-muted text-[12px]"><span>CGST (9%)</span><span>{{ inr(split().cgst) }}</span></div>
                <div class="flex justify-between text-muted text-[12px]"><span>SGST (9%)</span><span>{{ inr(split().sgst) }}</span></div>
              }
            ...
            <div class="flex flex-col gap-2">
              <span class="font-label-sm text-label-sm text-muted font-medium">Select Payment Method</span>
            ...
            @if (method() === 'UPI' && total() > 0) {
              <div class="p-3 bg-surface-container-low rounded-xl border border-primary/20 flex items-center justify-between gap-3">
                <div class="flex items-center gap-3 min-w-0">
                  ...
                  <div class="flex flex-col min-w-0"><span class="font-label-md text-label-md font-bold text-on-surface">Scan &amp; Pay {{ inr(total()) }}</span><span class="text-[11px] text-muted truncate">{{ vpa() }}</span></div>
                </div>
              </div>
            }
      ...
      @if (receipt(); as r) {
        <div class="space-y-4">
          ...
          <div class="text-body-md space-y-1">
            ...
            @if (r.discount) { <div class="flex justify-between text-primary pt-1 border-t border-outline-variant/20"><span>Coupon {{ r.couponCode }}</span><span>-{{ inr(r.discount) }}</span></div> }
            @if (r.gstRegistered) {
              <div class="pt-1 border-t border-outline-variant/20 text-[12px] text-muted space-y-0.5">
                <div class="flex justify-between"><span>Taxable value</span><span>{{ inr(r.taxable) }}</span></div>
                <div class="flex justify-between"><span>CGST (9%)</span><span>{{ inr(r.cgst) }}</span></div>
                <div class="flex justify-between"><span>SGST (9%)</span><span>{{ inr(r.sgst) }}</span></div>
                @if (r.gstin) { <div class="flex justify-between"><span>GSTIN</span><span class="font-mono">{{ r.gstin }}</span></div> }
              </div>
            }
...
export class QuickBill implements OnInit {
  ...
  applyCoupon() {
    ...
    this.toast.success(`${r.label} applied`);
  }
