...
import { dateKey, downloadText, fmt12, initials, inr, toCsv } from '../../../core/utils/time';
...
@Component({
  ...
  imports: [FormsModule, RouterLink, Topbar, Modal, DayPicker, LineChart],
  template: `
    <app-topbar>
      <div left class="flex items-center gap-4 flex-1 max-w-xl">
        ...
        <div class="hidden lg:flex items-center gap-2 bg-surface-container-low border border-outline-variant/40 rounded-lg px-3 py-1.5">
          ...
          <span class="font-label-md text-label-md text-on-surface font-semibold">This Month ({{ month }})</span>
        </div>
      </div>
      <ng-container right>
        <button type="button" (click)="openAdd()" class="px-3.5 py-1.5 rounded-lg bg-secondary-container text-on-secondary-container font-label-md text-label-md flex items-center gap-1.5 hover:opacity-90 active:scale-95 transition-all"><span class="material-symbols-outlined text-[18px]">person_add</span><span class="hidden sm:inline">Add New Staff</span></button>
        <a routerLink="/owner/quick-bill" class="hidden sm:flex px-3.5 py-1.5 rounded-lg bg-primary text-on-primary font-label-md text-label-md items-center gap-1.5 hover:bg-primary-container active:scale-95 transition-all"><span class="material-symbols-outlined text-[18px]">point_of_sale</span><span>Quick Bill</span></a>
        <button type="button" class="hidden md:block p-2 text-on-surface-variant hover:bg-surface-container rounded-lg transition-colors" title="Help" (click)="toast.info('Select a stylist to see their earnings and adjust commission')"><span class="material-symbols-outlined text-[20px]">help</span></button>
      </ng-container>
    </app-topbar>
          ...
          @if (selected(); as r) {
            <div class="lg:col-span-5 bg-surface-container-lowest rounded-2xl border border-outline-variant/30 shadow-md p-5 flex flex-col gap-5">
              ...
              <div class="grid grid-cols-2 gap-2.5">
                <button type="button" (click)="openCommission(r.staff)" class="py-2 px-3 rounded-xl border border-primary text-primary font-label-md text-label-md font-semibold hover:bg-primary/5 active:scale-95 transition-all flex items-center justify-center gap-1.5"><span class="material-symbols-outlined text-[18px]">percent</span><span>Adjust Commission %</span></button>
                <button type="button" (click)="openSchedule(r.staff)" class="py-2 px-3 rounded-xl bg-primary text-on-primary font-label-md text-label-md font-semibold hover:bg-primary-container active:scale-95 transition-all flex items-center justify-center gap-1.5 shadow-sm"><span class="material-symbols-outlined text-[18px]">calendar_add_on</span><span>Assign Schedule</span></button>
              </div>
              ...
              <div class="flex flex-col gap-3">
                <h4 class="font-headline-sm text-headline-sm font-bold text-on-surface">Bookings Today</h4>
                <div class="flex flex-col gap-2.5 max-h-[300px] overflow-y-auto pr-1 custom-scrollbar">
                  @for (b of todayBookings(); track b.id) {
                    ...
                    <p class="text-body-sm text-outline py-4 text-center">No bookings for {{ r.staff.name.split(' ')[0] }} today.</p>
                  }
                </div>
              </div>
    ...
    <app-modal [open]="commissionFor() !== null" title="Adjust Commission %" (closed)="commissionFor.set(null)">
      @if (commissionFor(); as s) {
        <div class="space-y-4">
          ...
          <div class="flex justify-end gap-3 pt-4 border-t border-outline-variant/20"><button type="button" [class]="ghost" (click)="commissionFor.set(null)">Cancel</button><button type="button" [class]="primary" (click)="saveCommission()">Save</button></div>
        </div>
      }
    </app-modal>
    ...
    <app-modal [open]="adding()" title="Add New Staff" (closed)="adding.set(false)">
      <form class="space-y-4" (ngSubmit)="saveNew()" #f="ngForm">
        <div><label [class]="label" for="ns-name">Full name</label><input id="ns-name" name="name" [class]="input" [(ngModel)]="nName" required placeholder="e.g., Amit Patel" /></div>
        <div class="grid grid-cols-2 gap-4">
          <div><label [class]="label" for="ns-phone">Phone</label><input id="ns-phone" name="phone" [class]="input" [(ngModel)]="nPhone" required placeholder="+91 98111 22233" /></div>
          <div><label [class]="label" for="ns-role">Role</label><select id="ns-role" name="role" [class]="input" [(ngModel)]="nRole">@for (r of roles; track r) { <option [value]="r">{{ r }}</option> }</select></div>
        </div>
        <div><label [class]="label" for="ns-comm">Commission %</label><input id="ns-comm" name="comm" type="number" min="0" max="100" [class]="input" [(ngModel)]="nComm" required /></div>
        <div class="flex justify-end gap-3 pt-4 border-t border-outline-variant/20"><button type="button" [class]="ghost" (click)="adding.set(false)">Cancel</button><button type="submit" [class]="primary" [disabled]="f.invalid">Add staff</button></div>
      </form>
    </app-modal>
  `,
})
export class StaffPerformance {
  ...
  protected readonly month = new Date().toLocaleDateString('en-IN', { month: 'long', year: 'numeric' });
  protected readonly payoutDate = new Date(new Date().getFullYear(), new Date().getMonth() + 1, 1).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' });
  ...
  toggleDuty(s: StaffMember) {
    ...
    this.toast.info(`${s.name} marked ${s.status === 'on-duty' ? 'off duty' : 'on duty'}`);
  }
  ...
  saveCommission() {
    ...
    this.toast.success(`${s.name}'s commission set to ${pct}%`);
  }
  ...
  saveSchedule() {
    ...
    this.toast.success(`${s.name}'s schedule updated`);
  }
  ...
  saveNew() {
    ...
    this.toast.success(`${m.name} added to your team`);
  }
