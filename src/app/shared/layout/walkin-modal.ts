...
@Component({
  ...
  template: `
    <app-modal [open]="ui.walkInModal()" title="Add Walk-in" (closed)="ui.walkInModal.set(false)">
      <form class="space-y-4" (ngSubmit)="submit()" #f="ngForm">
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label [class]="label" for="wi-name">Client name</label>
            <input id="wi-name" name="name" [class]="input" [(ngModel)]="name" required placeholder="e.g., Amit Saxena" />
          </div>
          <div>
            <label [class]="label" for="wi-phone">Mobile number</label>
            <input id="wi-phone" name="phone" [class]="input" [(ngModel)]="phone" placeholder="Optional" />
          </div>
        </div>
        <div>
          <label [class]="label" for="wi-service">Service</label>
        ...
        <div>
          <label [class]="label" for="wi-staff">Preferred stylist</label>
        ...
        <div class="flex items-center justify-end gap-3 pt-4 border-t border-outline-variant/20">
          <button type="button" [class]="ghost" (click)="ui.walkInModal.set(false)">Cancel</button>
          <button type="submit" [class]="primary" [disabled]="f.invalid">Add to queue</button>
        </div>
      </form>
    </app-modal>
  `,
...
export class WalkinModal {
  ...
  submit() {
    ...
    this.toast.success(`${this.name} added to the waiting queue`);
