  ...
  template: `
    <header class="sticky top-0 z-50 flex items-center justify-between w-full h-16 px-4 md:px-8 max-w-full bg-surface-container-lowest border-b border-outline-variant/20 shadow-sm no-print">
      ...
      <div class="flex items-center gap-2 md:gap-3">
        ...
        @if (!complete()) {
          <button type="button" (click)="saveDraft()" class="hidden sm:block text-body-sm font-label-lg px-3.5 py-1.5 rounded-lg border border-outline-variant/40 text-on-surface hover:bg-surface-container-low active:scale-[0.99] transition-all">Save Draft</button>
        }
        <button type="button" (click)="exit()" class="text-body-sm font-label-lg px-3.5 py-1.5 rounded-lg text-secondary hover:bg-error-container/20 active:scale-[0.99] transition-all">Exit Setup</button>
    ...
    <div class="md:hidden bg-surface-container-lowest px-4 py-2.5 border-b border-outline-variant/20 flex items-center justify-between no-print">
      <div class="flex items-center gap-2">
        <span class="text-xs font-semibold text-primary">Step {{ complete() ? 4 : active() }} of 4:</span>
